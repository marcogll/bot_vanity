...
Evolution API recibe mensajes de WhatsApp y los reenvía a Vanessa mediante webhook.

Configuración esperada:
- Instancia de Evolution conectada a WhatsApp.
- Evento `MESSAGES_UPSERT` habilitado.
- Webhook apuntando a `/webhook` de Vanessa.
- `apiKey` del webhook igual a `WEBHOOK_SECRET`.

Vanessa responde por Evolution usando:
